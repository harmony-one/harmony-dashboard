            ...
            <h1 v-if="isHrc721(address.id)">
              ...
              HRC721 Token:
               <span :style="bgStyle(isHrc721(address.id).name)">
                  {{ isHrc721(address.id).name }} (<b>{{ isHrc721(address.id).symbol }}</b
               >)
                </span>
            ...
            <section>
              <table class="explorer-table">
                ...
                <tr v-if="isHrc721(address.id)">
                  ...
                  <td>
                    {{ isHrc721(address.id).name + "(" + isHrc721(address.id).symbol + ")" }}
                  </td>
                </tr>
                ...
                <tr>
                  <td class="td-title">
                    {{ isHrc721(address.id) || isHrc20(address.id) ? "Contract" : "ID" }}
                  </td>
                  <td>
                    <Address
                        :bech32="address.id"
...
export default {
  ...
  methods: {
    ...
    async getHRC721Data() {
      ...
      try {
        this.hrc721TotalSupply = (await c.methods.totalSupply().call()).toString();
      } catch (e) {
      }
